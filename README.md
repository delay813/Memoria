...
- **跨记忆一致性**：`facts.json` 是「当前结论」的权威来源——当某条事实被纠正 / 淘汰时，会自动语义检索出相关的旧情景记忆并软遗忘（不再注入召回），让事实库优先于向量库，避免角色「旧事重提」自相矛盾。
...
- **每日命运大纲（剧本）**：旁白每天为每个角色写一条极简「今天做什么 + 要不要主动找你」的大纲（综合好感度、角色对用户的认识、当天行程，且严格贴合人设与事实）。大纲作为「命运」注入对话让角色遵从，并决定当天是否主动发消息；具体演绎仍交给角色自己。
...
│   │       └── chroma_db/
│   └── static/                   # 前端（chat.html + theme.css 主题样式 + 角色立绘/头像）
├── tests/                        # 单元测试
├── run.py                        # 跨平台启动入口(推荐)
...
pytest -q
```
> 仓库含 GitHub Actions（`.github/workflows/ci.yml`），每次 push / PR 会自动执行「依赖安装 + 静态检查（ruff）+ 语法检查 + 单元测试」。
...
| `MEMORY_ACCESS_SAVE_INTERVAL` | 60 | 召回访问计数落盘节流（秒），避免每轮对话都写注册表文件 |
...
| `GET` | `/agents/{id}/memory` | 指定角色的记忆卡（事实库 + 用户画像 + 关系状态） |
| `GET` | `/agents/{id}/topics` | 为指定角色生成开场话题 |
| `GET` | `/user` | 当前用户昵称 |
| `POST` | `/user` | 设置用户昵称，body：`{"nickname"}` |
| `GET` | `/achievements` | 全部成就及解锁状态 |
