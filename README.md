...
│   │   └── npc_01/
│   │       ├── 人格.md / 人格锚点.md
│   │       ├── chat.json / memory.json / state.json / inbox.json / facts.json
│   │       └── chroma_db/
...
> `agent/agents/` 与 `agent/world.json` 都是运行时生成的数据，已在 `.gitignore` 中排除；克隆仓库后首次运行会自动生成。
