...
class Agent:
    ...
    def __init__(self, agent_id, name, persona, description="", birthday=""):
    ...
    def reset(self):
        """初始化该角色: 清空对话历史/结晶记忆/好感度/心情/长期记忆/事实库/收件箱(保留人格内核与锚点)"""
        with self._lock:
            self.history = [{"role": "system", "content": self.persona}]
            self._favor = config.FAVOR_INITIAL
            self._mood = 0.0
            ...
            for f in (self.history_file, self.memory_file, self.state_file,
                      self.inbox_file, self.fact_file):
                try:
                    if os.path.exists(f):
                        os.remove(f)
                except Exception as e:
...
"""
    ...
    def _maybe_judge_favor_async(self):
        """每轮回复后, 后台即时结算一次好感度(调用方已持有 self._lock)"""
        if self._judging_favor:
            return
        self._judging_favor = True
        threading.Thread(target=self._run_favor_judge, daemon=True).start()

    def _run_favor_judge(self):
        try:
            with self._lock:
            ...
            delta = self._judge_favor(snapshot)
            with self._lock:
                if delta:
            ...
            print(f"[{self.name}] 好感度判断失败: {e}")
        finally:
            self._judging_favor = False

    def _judge_favor(self, recent):
        """判断最近一轮对话对好感度的影响(-8~+8); 用便宜模型, 失败退回主模型"""
        history_text = "\n".join(
...
"""
    ...
    def _build_recall_query(self, prompt):
        """用"当前消息 + 最近上下文"做检索, 更贴近人脑的联想式激活"""
        recent = [m for m in self.history if m.get("role") in ("user", "assistant")][-3:]
        text = " ".join(self._clean_text(m.get("content", "")) for m in recent)
        text = (text + " " + self._clean_text(prompt)).strip()
...
输出格式:
{{"user_profile":"...","conversation_summary":"...","persona_growth":"...","facts":["...","..."]}}
"""
        resp = _call_generation([{"role": "user", "content": prompt}])
        if not resp:
    ...
    def _should_compact(self):
        ...
        if user_turns < config.MEMORY_UPDATE_THRESHOLD:
            return False
        memory = self._load_memory()
        return user_turns - memory.get("last_compact_turns", 0) >= config.MEMORY_UPDATE_THRESHOLD

    def _maybe_compact_async(self):
        """需要压缩时, 丢到后台线程执行(调用方已持有 self._lock)"""
        if not self._should_compact() or self._compacting:
            return
    ...
    def _apply_compact(self, profile, summary, growth, facts):
        ...
        if len(self.history) > config.MEMORY_KEEP_TURNS * 2:
            ...
            for m in removed:
                if m.get("content") and _worth_remembering(m.get("content", "")):
                    ...
                    batch_metas.append({"agent": self.agent_id, "role": m.get("role", "")})
            if batch_texts:
    ...
    def chat_stream(self, prompt, world_context=None, greet_hint=None):
        ...
        try:
            ...
            # 3) 变长时异步压缩(摘要+画像+事实+人格成长+写长期记忆, 不阻塞回复)
            self._maybe_compact_async()

            # 4) 用户轮次先落盘
            self._save_history()

            # 5) 认知步骤: 读心 + 内心独白 + 心情变化(便宜模型, 可开关)
            ...
            messages = [{
                "role": "system",
                "content": self._build_system_message(recalled, facts, world_context, greet_hint, cognition),
            }]
